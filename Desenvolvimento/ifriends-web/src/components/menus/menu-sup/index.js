			...
			items={[
				{
					...
					label: (
						<Stack style={{ marginLeft: '0.5rem' }}>
							<strong> email@aluno.ifsp.edu.br</strong>
							<small>{localStorage.getItem('@user-name')}</small>
						</Stack>
					),
					icon: <IconPattern icon={AiOutlineUser} boxSize={30} size={30} />
				},
				{
					type: 'divider'
				},
				{
					key: '2',
          			label: <span> {t("menu-select-idiom-label")}: { locale === 'pt' ? "Português" : campos.language } </span>,
